...
done < <(find "$worktrees_root" -path '*/.agents/specs/*/spec.md' -type f 2>/dev/null | sort)
...
if (( ${#matching_specs[@]} == 1 )); then
  ...
  worktree_path="${existing_spec%%/.agents/specs/*}"
  ...
  spec_dir="$worktree_path/.agents/specs/$task_ref"
