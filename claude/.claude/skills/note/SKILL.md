...
3. **If a matching note exists:** append the new content under the most appropriate existing heading, or create a new heading if none fits. Use today's date as a subheading only if the content is time-sensitive (e.g., a log entry, a meeting note). Keep additions concise and in the existing style of the file.

4. **If no matching note exists:** create a new file named `Title.org` (title-cased, spaces allowed, no timestamp prefix) with this structure:
   ```org
   #+TITLE: Title
